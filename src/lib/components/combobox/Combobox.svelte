<script lang="ts" context="module">
  ...
  export function useComboboxContext(component: string): Readable<StateDefinition> {
    let context: Writable<StateDefinition> | undefined = getContext(COMBOBOX_CONTEXT_NAME)
  ...
  type TComboboxProps<TSlotProps extends {}, TAsProp extends SupportedAs> = TPassThroughProps<
    TSlotProps,
    TAsProp,
    'div'
  > & {
...
<script lang="ts">
  ...
  let activationTrigger: StateDefinition['activationTrigger'] = ActivationTrigger.Other
  let mode: StateDefinition['mode'] = multiple ? ValueMode.Multi : ValueMode.Single
  ...
  $: {
    if (defaultToFirstOption && activeOptionIndex === null && options.length > 0) {
  ...
  let api = writable<StateDefinition>({
    ...
    openCombobox() {
      ...
      let optionIdx = options.findIndex((option) => {
        ...
        let selected = match(mode, {
          ...
          [ValueMode.Multi]: () =>
            ($api.value as unknown[]).some((value) => $api.compare(value, optionValue))
        })
    ...
    goToOption(focus: Focus, id?: string, trigger?: ActivationTrigger) {
      ...
      if (disabled) return
      if (optionsRef && !$optionsPropsRef.static && comboboxState === ComboboxStates.Closed) {
        return
      }
  ...
  let openClosedState = writable<State>(computeOpenClosedState(comboboxState))

  useOpenClosedProvider(openClosedState)
  $: $openClosedState = computeOpenClosedState(comboboxState)

  $: activeOption =
    $api.activeOptionIndex === null ? null : (options[$api.activeOptionIndex]?.dataRef as any)
...
{#if name != null && value != null}
  ...
  {/each}
{/if}
<Render {...$$restProps} {as} {slotProps} use={[...use, forwardEvents]} name={'Combobox'}>
  <slot {...slotProps} />
</Render>
