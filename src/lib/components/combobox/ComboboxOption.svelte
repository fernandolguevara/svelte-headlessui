<script lang="ts" context="module">
  type TComboboxOptionProps<TSlotProps extends {}, TAsProp extends SupportedAs> = Omit<
...
<script lang="ts">
  ...
  let internalValue: Writable<Object | string | number | boolean> = writable(value)
  ...
  $: selected = match($api.mode, {
    ...
    [ValueMode.Multi]: () => ($api.value as unknown[]).some((v) => $api.compare(v, value))
  })
