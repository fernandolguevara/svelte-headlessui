<script lang="ts" context="module">
  type TComboboxInputProps<TSlotProps extends {}, TAsProp extends SupportedAs> = TPassThroughProps<
    TSlotProps,
    TAsProp,
    'input'
  > & {
...
<script lang="ts">
  ...
  function handleStateChange(state: ComboboxStates) {
    ...
    if (state === ComboboxStates.Closed && input.value != newValue) input.value = newValue
  }
  ...
  $: propsWeControl = {
    ...
    'aria-expanded': $api.disabled ? undefined : $api.comboboxState === ComboboxStates.Open,
